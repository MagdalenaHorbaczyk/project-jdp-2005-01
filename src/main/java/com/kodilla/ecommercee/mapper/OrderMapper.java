...
public class OrderMapper {
    ...
    public List<OrderDto> mapToOrderDtoList(final List<Order>orders) {
        return orders.stream()
                .map(order -> new OrderDto(order.getOrderId(),order.getOrderPositions(),
                        order.getCreationDate(),order.getStatus(),
                        order.getUser(), order.getDeliveryMethod(),
                        order.getPaymentMethod(),order.getDeliveryAddress()))
